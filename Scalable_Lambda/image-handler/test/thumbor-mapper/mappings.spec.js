...
describe("mapConvolution", () => {
    it("Should map the convolution matrix to the current edits", () => {
        ...
        // Assert
        expect(currentEdits.convolve).toEqual(expect.objectContaining({
            height: 3,
            width: 2,
            kernel: [1, 2, 3, 4, 5, 6],
        }));
...
describe("mapFill", () => {
    it("Should map resize fit and fill background rgb color with color object when color name string provided", () => {
        ...
        // Assert
        expect(currentEdits.resize).toEqual(expect.objectContaining({
            fit: "contain",
...
describe("mapResizeRatio", () => {
    it("Should replace width and height proportionally if they already exist", () => {
        ...
        const currentEdits = { resize: { width: 10, height: 10 } };
        const thumborMapper = new thumbor_mapper_1.ThumborMapper();
        // Act
...
describe("mapQuality", () => {
    it("Should map format and quality when it is a valid format", () => {
        ...
        const formatType = lib_1.ImageFormatTypes.JPG;
        const currentEdits = {};
        const thumborMapper = new thumbor_mapper_1.ThumborMapper();
        // Act
...
describe("mapStretch", () => {
    it("Should map resize fit to fill if not already set to inside", () => {
        ...
        // Assert
        expect(currentEdits.resize.fit).toEqual(lib_1.ImageFitTypes.FILL);
    });
    it("Should not map resize fit to fill if it is already set to inside", () => {
        // Arrange
        const currentEdits = { resize: { fit: lib_1.ImageFitTypes.INSIDE } };
        const thumborMapper = new thumbor_mapper_1.ThumborMapper();
        // Act
        thumborMapper["mapStretch"](currentEdits);
        // Assert
        expect(currentEdits.resize.fit).toEqual(lib_1.ImageFitTypes.INSIDE);
    });
});
describe("mapUpscale", () => {
...
describe("mapWatermark", () => {
    it("Should map overlayWith values with provided values if x and y pos are valid", () => {
        ...
        // Assert
        expect(currentEdits.overlayWith).toEqual(expect.objectContaining({
            bucket: "bucket",
            key: "key",
            alpha: "50",
            wRatio: "30",
